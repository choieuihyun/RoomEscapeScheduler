...
오른쪽의 "캘린더" 메뉴는 저장한 일정(F-15)과 감시 중인 매진 회차(F-16)를
날짜별로 겹쳐 보여주는 한 달 그리드 모달이다 — 새 서버 호출 없이 기존
`usePlans`/`useWatches` 데이터를 합쳐서 그린다(§4.38).

공유 링크는 `#s=`(그대로)와 `#z=`(deflate 압축) 두 형식을 **읽는다**.
`#z=` 는 같은 JSON 을 압축만 한 것이라 풀면 `#s=` 와 바이트가 같다 —
실측 1,583자 → 606자(§4.43). **쓰기는 `serialize.ts` 의 `SHARE_WRITE_PACKED`
스위치 뒤에 있고 지금은 꺼져 있다** — 옛 번들을 캐시한 사람이 `#z=` 를 못
읽기 때문이다. 읽기가 배포되고 **하루 지난 뒤** `true` 로 바꾸면 켜진다. 날짜를 클릭하면
그날 F-15 서버가 수집해 둔 모든 지점의 실제 회차 시간표도 같이 훑어볼 수
있다(§4.40) — 지점이 많은 날은 "더 보기"로 나눠 보여준다.
